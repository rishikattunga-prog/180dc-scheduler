...
export default function App() {
  ...
  useEffect(() => {
    ...
    /* live sync from other devices. We stamp every write with a signature and
       ignore the realtime echo of our own write so a fresh booking is never
       clobbered by its own round-trip. */
    const unsub = storageSubscribe(STORAGE_KEY, (val) => {
      const incoming = typeof val === "string" ? JSON.parse(val) : val;
      if (incoming && incoming.__sig && incoming.__sig === lastSigRef.current) return; // our own echo
          ...
          {events.map((e) => {
            ...
            return (
              <div key={e.id} className="iv-event card">
                ...
                <p className="pick-label">Select a start time:</p>
                <div className="cohort-card-grid">
                  {open.map((c) => {
                    ...
                    return (
                      <button key={c.id}
                        className={`cohort-box${picked ? " is-selected" : ""}`}
                        onClick={() => { setEventId(e.id); setCohortId(c.id); setError(""); }}>
                        <b>{c.start}</b>
                        <span className="cohort-left">{left === 1 ? "1 spot left" : `${left} spots left`}</span>
                      </button>
                    );
                  })}
                </div>
...
function InterviewAdmin({ data, save, logout }) {
  ...
  /* fall back to the first event whenever the chosen one isn't present (e.g. data loaded after mount) */
  const activeEvent = events.find((e) => e.id === activeEventId) || events[0] || null;
  useEffect(() => {
    if ((!activeEventId || !events.some((e) => e.id === activeEventId)) && events[0]) {
